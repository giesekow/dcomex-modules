...
nifti_reg_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), "lib", "niftyreg", "bin")
...
def niftireg_affine_registration(moving_path,
    ...
    cmd = os.path.join(nifti_reg_dir, 'reg_aladin')
    cmd += ' -flo ' + moving_path
    cmd += ' -ref ' + fixed_path
...
def niftireg_nonrigid_registration(moving_path,
    ...
    cmd = os.path.join(nifti_reg_dir, 'reg_f3d')
    cmd += ' -flo ' + moving_path + ' '
    cmd += '-ref ' + fixed_path + ' '
...
def niftireg_transform(moving_path,
    ...
    # Register to reference file
    cmd = os.path.join(nifti_reg_dir, 'reg_resample')
    cmd += ' -flo ' + moving_path + ' '
    cmd += '-ref ' + fixed_path + ' '
    if not cpp:
        cmd += '-aff ' + transform_path + ' '
